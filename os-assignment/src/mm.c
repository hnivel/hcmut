...
pthread_mutex_t frame_list_lock = PTHREAD_MUTEX_INITIALIZER;
/*
...
int alloc_pages_range(struct pcb_t *caller, int req_pgnum,
    ...
    */
    ...
    /*Synchronization with frame_list_lock*/
...
int init_mm(struct mm_struct *mm, struct pcb_t *caller) {
    ...
    if (mm->pgd == NULL) {
        free(vma0); // Free previously allocated memory
        return -1;  // Error: Memory allocation failed
    }

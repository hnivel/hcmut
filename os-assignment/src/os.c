...
static void *cpu_routine(void *args) {
    ...
    while (1) {
        ...
        if (proc == NULL) {
             ...
             * ready queue */
            proc = get_proc();
        } else if (proc->is_killed) {
/* The process is killed, we need to load new process */
#ifdef SELFDBG
...
static void sort_ld_processes(void) {
    for (int i = 0; i < num_processes - 1; i++) {
        for (int j = i + 1; j < num_processes; j++) {
            if (ld_processes.start_time[i] > ld_processes.start_time[j]) {

                unsigned long temp_time = ld_processes.start_time[i];
                ld_processes.start_time[i] = ld_processes.start_time[j];
                ld_processes.start_time[j] = temp_time;

                char *temp_path = ld_processes.path[i];
                ld_processes.path[i] = ld_processes.path[j];
                ld_processes.path[j] = temp_path;
#ifdef MLQ_SCHED
                unsigned long temp_prio = ld_processes.prio[i];
                ld_processes.prio[i] = ld_processes.prio[j];
 ...
 * -----------------------
 * Return the time slot of the system
 *
 * returns: time slot
 */
int get_time_slot() { return time_slot; }

...
public class HosePriceFetchLib {
    private Map<String, HoseData> stockData = new HashMap<>();
    private String filePath;
    private Random random = new Random();

    public HosePriceFetchLib(String filePath) {
        this.filePath = filePath;
        loadStockData("HOSE"); // Load only HOSE stocks
    }
    ...
    private void loadStockData(String exchange) {
        try {
            ...
            for (String stockCode : stockList.keySet()) {
                ...
                stockData.put(stockCode, new HoseData(stockCode, price, volume, timestamp));
            }
    ...
    public List<HoseData> getPrices(List<String> codes) {
        loadStockData("HOSE"); // Refresh data before fetching
        List<HoseData> result = new ArrayList<>();

...
class StructDecl(Decl):
    ...
    def __str__(self):
        members_str = ", ".join(str(m)
                                for m in self.members) if self.members else ""
...
class BlockStmt(Stmt):
    ...
    def __str__(self):
        stmts_str = (
            ", ".join(str(s)
                      for s in self.statements) if self.statements else ""
        )
...
class ForStmt(Stmt):
    ...
    def __init__(
        ...
        self.condition = condition
        # Expr or None (PrefixOp, PostfixOp, or AssignExpr)
        self.update = update
        self.body = body
...
class StructLiteral(Expr):
    ...
    def __str__(self):
        values_str = ", ".join(str(v)
                               for v in self.values) if self.values else ""
